...
const LeadManagement: React.FC = () => {
    ...
    if (workspaceId) {
      ...
      if (prevWorkspaceIdRef.current !== workspaceId) {
        ...
        if (prevWorkspaceIdRef.current) {
          ...
          dispatch(invalidateLeadsCacheOnWorkspaceChange(workspaceId));
        }
  ...
  useEffect(() => {
    if (workspaceData) {
      ...
      setTotalLeads(workspaceData.count || 0);
      setHasMore((currentPage + 1) * 12 < (workspaceData.count || 0));
    }
  ...
  useEffect(() => {
    if (!isLoadingLeads && workspaceData?.data) {
      ...
      // Mark duplicates
      const processedLeads = fetchedLeads.map((lead: any) => ({
        ...lead,
  ...
  const lastLeadElementRef = useCallback((node: HTMLElement | null) => {
  ...
  }, [isLoadingLeads, hasMore, loadMoreLeads]);
          ...
          <div className="md:bg-white dark:md:bg-black bg-white dark:bg-black flex items-center justify-between col-start-1 col-end-7 p-3 md:p-0">
            <div className="flex gap-2 items-center">
              <div className="md:hidden lg:hidden">
                <SquareCode className="h-6 w-6 text-black dark:text-white" />
              </div>
              <CardTitle className="flex mr-2 text-md md:text-xl lg:text-2xl text-black dark:text-white">
                Lead Management
              </CardTitle>
            </div>
            ...
            <Button
              ...
              className="md:hidden lg:hidden bg-white dark:bg-black text-black dark:text-white border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-900"
            >
        ...
        ${lead.is_phone_valid ? "text-emerald-800" : "text-red-800"}`}
          ...
          <div className="flex justify-between items-center mt-4">
            <Button
              ...
              disabled={currentPage === 0 || isLoadingLeads || isFetchingLeads}
              variant="outline"
            >
              Previous
            </Button>
            <span>
              Page {currentPage + 1} of {Math.ceil(totalLeads / 12) || 1}
            </span>
            <Button
              onClick={handleNextPage}
              disabled={!hasMore || isLoadingLeads || isFetchingLeads}
              variant="outline"
            >
              Next
            </Button>
